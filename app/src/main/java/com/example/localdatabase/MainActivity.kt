...
class HomeworkActivity : AppCompatActivity() {
        ...
        if (result.data != null) {
            when (result.resultCode) {
                AddHomeworkActivity.RESULT_ADD -> {
                    val homework =
                        result.data?.getParcelableExtra<Homework>(AddHomeworkActivity.EXTRA_HOMEWORK) as Homework
                    adapter.addItem(homework)
                    binding.rvHomework.smoothScrollToPosition(position = adapter.itemCount - 1)
                ...
                AddHomeworkActivity.RESULT_UPDATE -> {
                    val homework =
                        result.data?.getParcelableExtra<Homework>(AddHomeworkActivity.EXTRA_HOMEWORK) as Homework
                    val position =
                        result.data?.getIntExtra(AddHomeworkActivity.EXTRA_POSITION, defaultValue = 0) as Int
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            ...
            val list =
                savedInstanceState.getParcelableArrayList<Homework>(EXTRA_STATE)
            if (list != null) {
